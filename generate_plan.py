...
def generate_plan(boundary, obstacles, waypoints, center):
  ...
  obst = []
  for pt in obstacles:
    (x, y) = utm.to_latlon(xc + pt[0][0] / 3.281, yc + pt[0][1] / 3.281, zc,
                           dc)
    obst.append({
        "circle": {
            "center": [x, y],
  ...
  for i in range(len(waypoints)):
    ...
    item['doJumpId'] = i + 1
    (x, y) = utm.to_latlon(xc + waypoints[i][0] / 3.281,
                           yc + waypoints[i][1] / 3.281, zc, dc)
    item['params'] = [(15 if i == 0 else 0), 0, 0, None, x, y, 50]
    wayp.append(item)
  plan['mission']['items'] = wayp
  plan['mission']['plannedHomePosition'] = [
      center['latitude'], center['longitude'], 50
  ]
